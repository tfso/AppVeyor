...
(function (Appveyor) {
    ...
    var BuildVersion = (function () {
        ...
        BuildVersion.prototype.hasBuild = function () {
    ...
    var BuildWorker = (function () {
        ...
        BuildWorker.addArtifact = function (name, source, filename, type, cb) {
            ...
            BuildWorker.getInstance()
                ...
                _this.addMessage('Posting of artifact ' + name + ' to API was successful');
                _this.addMessage('Directory: ' + dir);
                _this.addMessage('Filename: ' + filename || location.base);
                process.stdout.write('\u001b[32mOK\u001b[39m\n');
                // we have a uploadUrl we can upload our artifact
                switch (type) {
                    case ArtifactType.Zip:
                        _this.addMessage("Zipping with cmd: 7z a " + path.normalize(os.tmpdir() + '/sencha-build/' + name + ".zip") + " " + dir);
                        ...
                        proc.exec("7z a " + path.normalize(os.tmpdir() + '/sencha-build/' + name + ".zip") + " *", { cwd: dir, env: process.env }, function (err, stdout, stderr) {
                            ...
                            _this.addMessage('Artifact size: ' + stat.size);
                            process.stdout.write('\u001b[32mOK\u001b[39m\n');
                            ...
                            BuildWorker.getInstance()
                                ...
                                process.stdout.write('\u001b[32mOK\u001b[39m\n');
                                _this.addMessage('Uploaded artifact ' + name + ' from ' + dir);
                                cb(null);
                            });
                        });
                        break;
