...
(function (Appveyor) {
    ...
    var BuildWorker = (function () {
            ...
            get: function () {
                ...
                return {
                    post: function (path, content, cb) { if (_this._request != null)
                        _this._request.post(path, content, cb); },
                    upload: function (path, filename, cb) { if (_this._request != null)
                        _this._request.sendFile(path, filename, cb); }
                };
            },
            enumerable: true,
            configurable: true
        ...
        BuildWorker.addArtifact = function (name, location, file, type) {
            BuildWorker.getInstance()
                .request
                .post('api/artifacts', { name: name, path: location, fileName: file || name, type: (type ? ArtifactType[type] : ArtifactType[ArtifactType.Auto]) }, function (err, response, body) {
                console.log(response.statusCode + ': ' + JSON.stringify(body));
                proc.exec("7z a " + path.normalize(os.tmpdir() + '/sencha-build/' + name + ".zip") + " " + path.normalize(location), { env: process.env }, function (err, stdout, stderr) {
                    if (err)
                        console.error(err);
                    console.log(stdout);
                    BuildWorker.getInstance()
                        .request
                        .upload(body, path.normalize(os.tmpdir() + '/sencha-build/' + name + ".zip"), function (err, res, body) {
                        if (err)
                            console.error(err);
                        console.log(response.statusCode + ': ' + JSON.stringify(body));
                    });
