        ...
        Module.prototype.build = function (options, callback) {
            ...
            var execute = new Promise(function (resolve, reject) {
                ...
                patch_version_1.default(_this.location, newversion, null, null, function () {
                    var err, cmd = proc.spawn(Sencha.cmd || 'sencha.exe', [(_this.type == ModuleType.Package ? 'package' : 'app'), 'build', (_this.type == ModuleType.Application ? 'production' : '')], { cwd: path.dirname(_this.location), env: process.env });
                    cmd.stdout.on('data', function (data) {
                        _this.output(data);
                    });
                    cmd.stderr.on('data', function (data) {
