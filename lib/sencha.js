...
(function (Sencha) {
    ...
    var Workspace = (function (_super) {
        ...
        Workspace.prototype.install = function (callback) {
            var _this = this;
            var execute = new Promise(function (resolve, reject) {
                var err, cmd = proc.spawn(Sencha.cmd || 'sencha.exe', ['framework', 'add', 'ext', _this.sdk || "ext"], { cwd: _this.workspace, env: process.env });
                cmd.stdout.on('data', function (data) {
                    _this.output(data); // this.emit('stdout', data.toString().replace(/\n/gi, ""));
                });
                cmd.stderr.on('data', function (data) {
                ...
                _this
                    ...
                    async.everyLimit(modules, 1, function (module, cb) {
                        if (options.buildOnly == null || module.type == options.buildOnly) {
                            ...
                            _this.emit('stdout', '\n');
                            _this.emit('Ignoring ' + module.name);
                            cb("", true);
                        }
                    }, function (result) {
                        if (err) {
        ...
        Module.prototype.open = function (callback) {
            ...
            var readProperties = new Promise(function (resolve, reject) {
                ...
                fsp
                    ...
                    _this.publish = typeof json.publish == 'boolean' ? json.publish : true;
                })
                    .then(function () {
                    resolve();
                })
