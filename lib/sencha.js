...
(function (Sencha) {
    ...
    var Workspace = (function (_super) {
        ...
        Workspace.prototype.publish = function (callback) {
            var _this = this;
                    ...
                    modules.forEach(function (module) {
                        switch (module.type) {
                            case ModuleType.Application:
                                appveyor_1.default.BuildWorker.addMessage('Adding artifact for application ' + module.name + ' located at ' + path.normalize(buildPath + 'production/' + module.name));
                                proc.exec("7z a " + module.name + ".zip " + path.normalize(buildPath + '/production/' + module.name + '/*'), { cwd: _this.workspace, env: process.env }, function (err, stdout, stderr) {
                                    if (err)
                                        console.error(err);
                                    console.log(stdout);
                                    appveyor_1.default.BuildWorker.addArtifact(module.name, _this.workspace, module.name + ".zip", appveyor_1.default.ArtifactType.Zip);
                                });
                                break;
    ...
    var Module = (function (_super) {
        ...
        Module.prototype.build = function (callback) {
            ...
            var execute = new Promise(function (resolve, reject) {
                ...
                var err, cmd = proc.spawn(Sencha.cmd || 'sencha.exe', [(_this.type == ModuleType.Package ? 'package' : 'app'), 'build'], { cwd: path.dirname(_this.location), env: process.env });
                cmd.stdout.on('data', function (data) {
                    _this.output(data);
                });
                cmd.stderr.on('data', function (data) {
