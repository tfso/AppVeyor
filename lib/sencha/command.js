...
class Command extends events.EventEmitter {
    ...
    static addRepository(name, url, callback) {
        var execute = new Promise((resolve, reject) => {
            var err, output = [], cmd = proc.spawn(Command.path || 'sencha.exe', ['repository', 'add', name, url], { env: process.env });
    ...
    static execute(args, output, cwd) {
    ...
    static install(url, destination) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                appveyor_1.default.BuildWorker.addMessage('Installed Sencha Cmd at ' + localStorage);
