...
program
    ...
    .action((options) => {
        ...
        if (options.jsb && options.jsb.length > 0) {
            ...
            var err,
                cmd = proc.spawn(sencha.Command.path || 'sencha.exe', ['build', '-p', path.normalize(options.jsb), '-d', path.normalize(options.destination)], { cwd: options.path, env: process.env });
